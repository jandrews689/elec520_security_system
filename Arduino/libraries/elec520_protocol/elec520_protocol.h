...
struct FloorNode {
  ...
  uint8_t  rssi        = 0;     // NEW: f/{f}/rsi (BYTE 0..255)
...
// Keeping signatures to avoid breaking callers; simple implementations provided.
 ...
 * Accepts ESP-NOW styles you described:
 *  - "f/<f_id>/rsi:<val>"
 * Also tolerant of:
 *  - "f/<f_id>/rsi <val>", "f/<f_id>/rsi=<val>",
